...
import { getHighScore, setHighScore, getSettings, getLastCompletedLevel, setLastCompletedLevel, shouldShowTutorial } from "../save/saveManager";
...
export default class DodgeGame extends BaseScene {
  ...
  create() {
    ...
    if (!this._returnData?.skipTutorialGate && shouldShowTutorial()) {
      this.scene.start(SCENE_KEYS.tutorial, { returnTo: SCENE_KEYS.game, returnData: { skipTutorialGate: true } });
      return;
    }
  ...
  showGameOver() {
    ...
    this.gameOverPanel = this.add
      .rectangle(GAME_WIDTH / 2, 220, 760, 410, 0x0d1823, 0.94)
    ...
    const buildSummary = this.ownedPerks.length ? this.ownedPerks.join(", ") : "Base build";
